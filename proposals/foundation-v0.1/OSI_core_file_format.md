...
| `dialect` | string | No | SQL dialect used for all expressions in this document (defaults to `ANSI_SQL` if not specified) |
...
| `expression` | string | Yes | Scalar expression in the document's dialect |
...
The expression is a simple string containing the expression in the dialect specified at the semantic model level.  Expressions are expected to be expressible as strings, in non-SQL dialects there must be a string representation of it.  There should be a representation that is human readable.
...
- Must be written in the dialect specified at the document level
...
| `expression` | string | Yes | Aggregate SQL expression in the dialect specified at the document level |
...
The expression is a simple string containing SQL in the dialect specified at the semantic model level.
...
- **1.1** (2026-01-29): Dialect simplification
  - **BREAKING CHANGE**: Moved dialect specification from expression level to document level
  - Each semantic model now specifies a single dialect
  - Expressions are now simple strings instead of multi-dialect objects
  - Improved clarity and reduced complexity for implementations
