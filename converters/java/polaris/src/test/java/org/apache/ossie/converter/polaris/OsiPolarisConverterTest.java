...
class OsiPolarisConverterTest {
    ...
    void testParseMinimalModel() {
        ...
        assertEquals(1, model.getSemanticModel().size());

        SemanticModel sm = model.getSemanticModel().get(0);
        assertEquals("test_model", sm.getName());
        assertEquals("sales analysis", sm.getAiContext());
    ...
    void testParseDatasetFields() {
        ...
        assertEquals(Field.DataType.DECIMAL, computed.getDatatype());
        assertEquals("quantity * unit_price",
                computed.getExpression().getDialects().get(0).getExpression());
    }
    ...
    void testParseTimeDimension() {
        ...
        assertEquals(Field.DataType.DATE, orderDate.getDatatype());
        assertTrue(orderDate.getDimension().getIsTime());
    }
    ...
    void testYamlGenerationRoundTrip() {
        ...
        assertTrue(yaml.contains("primary_key:"));
        ...
        assertTrue(yaml.contains("from_columns:"));
        ...
        assertEquals("sales analysis", reparsed.getSemanticModel().get(0).getAiContext());
        assertEquals(2, reparsed.getSemanticModel().get(0).getDatasets().size());
    }
    ...
    void testImporterMapTableToDataset() throws Exception {
        ...
        assertEquals(Field.DataType.INTEGER, ds.getFields().get(0).getDatatype());
        assertEquals(Field.DataType.STRING, ds.getFields().get(1).getDatatype());
        assertEquals(Field.DataType.DATE_TIME_TZ, ds.getFields().get(2).getDatatype());
        assertTrue(ds.getFields().get(2).getDimension().getIsTime());
        assertEquals(Field.DataType.DECIMAL, ds.getFields().get(3).getDatatype());
        assertEquals(Field.DataType.DATE_TIME, ds.getFields().get(4).getDatatype());
        assertTrue(ds.getFields().get(4).getDimension().getIsTime());
        assertEquals(Field.DataType.OPAQUE, ds.getFields().get(5).getDatatype());
        ...
        assertEquals(DialectExpression.Dialect.ANSI_SQL,
                ds.getFields().get(0).getExpression().getDialects().get(0).getDialect());
        assertEquals("test_user", new ObjectMapper().readTree(
                ds.getCustomExtensions().get(0).getData()).get("owner").asText());
    ...
    void testDatatypePrecedenceAndLegacyFallbacks() throws Exception {
        ...
        typedStringWithTimeRole.setDatatype(Field.DataType.STRING);

        Field typedLocalTimestamp = makeField("local_timestamp", false);
        typedLocalTimestamp.setDatatype(Field.DataType.DATE_TIME);
        ...
        exactConflict.setDatatype(Field.DataType.STRING);
        exactConflict.setCustomExtensions(Collections.singletonList(
                makeExtension("POLARIS", "{\"iceberg_type\":\"int\"}")));

        Field opaqueWithoutExtension = makeField("opaque_id", false);
        opaqueWithoutExtension.setDatatype(Field.DataType.OPAQUE);

        Field decimalWithoutExtension = makeField("amount", false);
        decimalWithoutExtension.setDatatype(Field.DataType.DECIMAL);
    ...
    void testTypeInference() throws Exception {
        ...
        model.setSemanticModel(Collections.singletonList(sm));
    ...
    void testEmptyModel() {
        ...
        assertEquals("0.2.0.dev0", model.getVersion());
        assertTrue(model.getSemanticModel().isEmpty());
    }
    ...
    private Field makeField(String name, boolean isTime) {
        Field f = new Field();
        f.setName(name);
        f.setExpression(makeExpression(name));
        if (isTime) {
            Dimension dimension = new Dimension();
            dimension.setIsTime(true);
    ...
    private Expression makeExpression(String expressionValue) {
        DialectExpression dialectExpression = new DialectExpression();
        dialectExpression.setDialect(DialectExpression.Dialect.ANSI_SQL);
        dialectExpression.setExpression(expressionValue);

        Expression expression = new Expression();
        expression.setDialects(Collections.singletonList(dialectExpression));
        return expression;
    }

    private CustomExtension makeExtension(String vendorName, String data) {
