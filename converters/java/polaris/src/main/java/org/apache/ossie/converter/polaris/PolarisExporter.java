...
public class PolarisExporter {
    ...
    private JsonNode inferIcebergType(Field field, AtomicInteger nextNestedId) {
        String fieldDatatype = field.getDatatype() == null ? null : field.getDatatype().toString();
        ...
        if (exactType != null) {
            ...
            if (fieldDatatype != null
                    && !Objects.equals(fieldDatatype, extensionDatatype)) {
                IcebergTypeMapper.warn(
                        field.getName(),
                        "datatype '" + fieldDatatype + "' conflicts with exact Iceberg type '"
        ...
        JsonNode portableType = IcebergTypeMapper.toDefaultIcebergType(fieldDatatype);
        if (portableType != null) {
            if ("Decimal".equals(fieldDatatype)) {
                IcebergTypeMapper.warn(
                        field.getName(),
        ...
        if (fieldDatatype != null) {
            if ("Opaque".equals(fieldDatatype)) {
                IcebergTypeMapper.warn(
                        field.getName(),
                ...
                IcebergTypeMapper.warn(
                        field.getName(),
                        "unrecognized Ossie datatype '" + fieldDatatype
                                + "'; using legacy inference");
            }
        }
        ...
        if (field.getDimension() != null
                && Boolean.TRUE.equals(field.getDimension().getIsTime())) {
