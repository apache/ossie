"""Shared helpers for the OSI <-> Databricks Metric View converters.
...
# Metric View join cardinalities (the only two values v1.1 defines). OSI has no
...
def read_stash(obj):
    """Return the DATABRICKS stash dict on an OSI object, or {} if absent.
...
def pick_expression(osi_expression):
    """Choose the SQL string for an OSI expression: DATABRICKS, else ANSI_SQL.
...
def merge_description(description, ai_context):
    ...
    The OSI schema allows ai_context to be either a string or an object. A string
