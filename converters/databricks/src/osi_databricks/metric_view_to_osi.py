"""Convert a Databricks Unity Catalog Metric View (v1.1) to an OSI semantic model.

Pure offline conversion. Accepts a Metric View (one `source` with a nested `joins`
tree). Metric View features OSI has no native field for -- filter, window, format,
...
A join condition an OSI relationship cannot represent (a non-equi or cross join) is
rejected, not stashed. See README.md.

Usage (CLI):
    osi-databricks import -i view.yaml [-o model.yaml] [--name NAME]
"""
...
# Metric View fields with no native OSI home -> stashed verbatim.
...
def _is_wildcard(col):
    """A wildcard column (`expr: source.*`) is projected without a `name`; OSI has
...
def convert_metric_view_to_osi(mv_yaml_str, model_name=None):
    """Parse Metric View v1.1 YAML and return OSI semantic model YAML (string)."""
...
def _convert_view(view, model_name):
    ...
    # silently through to OSI and only erroring on a later re-export.
    ...
    def walk(parent_name, parent_alias, joins):
        for join in joins or []:
            ...
            # -- recovering key info OSI would otherwise lack. Only a many_to_one join
    ...
    for dim in (view.get("dimensions") or view.get("fields") or []):
        if _is_wildcard(dim):
            _warn("dimension", f"wildcard column '{dim.get('expr')}' has no OSI field "
    ...
    for m in view.get("measures", []) or []:
        if _is_wildcard(m):
            _warn("measure", f"wildcard measure '{m.get('expr')}' has no OSI metric "
                             f"representation; skipped")
            continue
...
def _convert_join(join, parent_name, parent_alias, child):
    if not join.get("using") and not join.get("on"):
        raise ConversionError(
            ...
            f"condition-less (cross) joins have no OSI relationship representation."
        )
    ...
    if raw_on is not None:
        raise ConversionError(
            ...
            f"that an OSI relationship cannot represent. OSI joins are equi-joins of simple "
    ...
    # Cardinality (default many_to_one) decides the OSI direction, since `from` is
...
def _convert_dimension(dim, alias_to_dataset, fact_name):
    ...
    ds_name, osi_expr = _resolve_column(expr, alias_to_dataset, fact_name)

    field = {
        "name": name,
        "expression": {"dialects": [{"dialect": DIALECT_DATABRICKS, "expression": osi_expr}]},
    }
    if dim.get("comment"):
