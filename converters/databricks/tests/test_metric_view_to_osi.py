"""Tests for the Databricks Metric View -> OSI importer."""

import pytest

from osi_databricks import ConversionError
...
def test_fixtureB_import_matches_expected():
    out = importer.convert_metric_view_to_osi(load_fixture("fixtureB_metric_view.yaml"))
    assert canon(parse(out)) == canon(parse(load_fixture("fixtureB_osi.yaml")))
...
def test_fields_is_accepted_as_alias_for_dimensions():
    ...
    osi = parse(importer.convert_metric_view_to_osi(mv))
    fields = osi["semantic_model"][0]["datasets"][0].get("fields", [])
    assert [f["name"] for f in fields] == ["region"]


def test_unsupported_version_rejected():
    with pytest.raises(ConversionError):
        importer.convert_metric_view_to_osi("version: '0.1'\nsource: c.s.t\n")
...
def test_both_dimensions_and_fields_present_warns_and_uses_dimensions():
    ...
    with warnings.catch_warnings(record=True) as caught:
        ...
        osi = parse(importer.convert_metric_view_to_osi(mv))
    names = [f["name"] for f in osi["semantic_model"][0]["datasets"][0].get("fields", [])]
    assert names == ["kept"]
    assert any("fields" in str(w.message) and "ignored" in str(w.message) for w in caught)


def test_stash_written_at_each_level():
    osi = parse(importer.convert_metric_view_to_osi(load_fixture("fixtureB_metric_view.yaml")))
...
def test_complex_joined_dimension_filed_under_join_dataset():
    ...
    osi = parse(importer.convert_metric_view_to_osi(mv))
    cust = next(d for d in osi["semantic_model"][0]["datasets"] if d["name"] == "cust")
    assert any(f["name"] == "full" for f in cust.get("fields", []))


def test_non_equi_on_rejected():
    """OSI relationships are equi-joins (from_columns/to_columns required, minItems 1), so a
    non-equi `on` has no OSI representation and is rejected on import (rather than emitting
    a relationship with empty column lists, which is invalid per the OSI schema)."""
    mv = (
        "version: '1.1'\n"
        "source: c.s.fact\n"
        "joins:\n"
...
def test_complex_equi_on_rejected():
    ...
    into from/to columns, so it's rejected rather than producing schema-invalid OSI with
    empty column lists."""
...
def test_one_to_many_join_flips_from_to_and_stashes_source():
    """A one_to_many MV join becomes an OSI relationship with the MANY side as `from`
...
def test_at_most_one_match_recovers_unique_key():
    ...
    unique_keys entry on the joined dataset (recovering key info OSI would lack)."""
    mv = (
        "version: '1.1'\nsource: c.s.orders\n"
        "joins:\n- name: customer\n  source: c.s.customer\n"
    ...
    osi = parse(importer.convert_metric_view_to_osi(mv))
    cust = next(d for d in osi["semantic_model"][0]["datasets"] if d["name"] == "customer")
...
def test_sql_source_name_defaults_to_metric_view():
    ...
    osi = parse(importer.convert_metric_view_to_osi(mv))
    assert osi["semantic_model"][0]["name"] == "metric_view"
...
def test_measure_rewrite_with_regex_special_name():
    ...
    osi = parse(importer.convert_metric_view_to_osi(mv, model_name=r"a\1b"))
    expr = osi["semantic_model"][0]["metrics"][0]["expression"]["dialects"][0]["expression"]
...
def test_malformed_yaml_raises_conversion_error():
    ...
    with pytest.raises(ConversionError, match="Invalid YAML"):
        importer.convert_metric_view_to_osi("source: c.s.t\njoins: [oops\n")
...
def test_boollike_string_values_stay_strings_for_a_yaml_1_1_reader():
    ...
    field = yaml.safe_load(osi_out)["semantic_model"][0]["datasets"][0]["fields"][0]
...
def test_fact_qualifier_variants_in_on_decompose():
    ...
    for cond in (
        ...
        rel = parse(importer.convert_metric_view_to_osi(
...
def test_multi_column_on_with_bare_and_tablename_fact():
    ...
    rel = parse(importer.convert_metric_view_to_osi(mv))["semantic_model"][0]["relationships"][0]
...
def test_whitespace_source_part_rejected():
    ...
    with pytest.raises(ConversionError, match="3-part"):
        importer.convert_metric_view_to_osi("version: '1.1'\nsource: cat . sch . tbl\n")
...
def test_with_paren_subquery_source_accepted():
    ...
    osi = parse(importer.convert_metric_view_to_osi(mv))
    assert osi["semantic_model"][0]["datasets"][0]["source"].startswith("WITH(")
...
def test_using_join_emits_no_yaml_anchor():
    ...
    output has no YAML anchor/alias (`&id`/`*id`) -- safer for other OSI consumers."""
    out = importer.convert_metric_view_to_osi(
