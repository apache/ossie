...
def test_relationship_column_count_mismatch_raises():
    document = OSIDocument(
        semantic_model=[
            OSISemanticModel(
                name="m",
                datasets=[
                    OSIDataset(name="orders", source="db.s.orders"),
                    OSIDataset(name="customers", source="db.s.customers"),
                ],
                relationships=[
                    OSIRelationship(
    ...
    with pytest.raises(ValueError, match="from_columns.*to_columns.*same length"):
        OSIToWisdomConverter().convert(document, exported_at="2026-07-10T00:00:00+00:00")
