...
| `field.dimension` | a `dimensions[]` entry | Import always emits the block, because a Cube `dimensions:` entry *is* a dimension and the block's absence is what other converters read as "not one" — the Snowflake converter classifies a field without it as a fact regardless of datatype, so omitting it made every non-time dimension a Cortex Analyst fact. Left empty for a non-time dimension, so the consumer applies the spec's default rather than this converter asserting `is_time: false`. |
