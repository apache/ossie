...
| dataset | `views/<name>.view.yaml` | |
| `dataset.source` `catalog.schema.table` / `schema.table` | view `catalog` + `schema` + `table_name` | `table_name` left implicit when it matches the view name. |
...
| `dataset.primary_key` (composite) | view `custom_compound_primary_key_sql` | Import: multiple `primary_key: true` dimensions also form a composite key. |
| field | dimension | Export: names are sanitized to Omni's `[a-z][a-z0-9_]*`; a collision is an error. |
...
| relationship | `relationships.yaml` entry | `from`(many) -> `join_from_view`, `to`(one) -> `join_to_view`, `relationship_type: many_to_one`, columns -> `on_sql` equi-join. |
...
topics (verbatim, minus the natively-mapped description/AI context), view
extras (`label`, `hidden`, `tags`, view-level `filters:`, ...), dimension
extras (`format`, `group_label`, exact `timeframes`, original `sql`, ...),
join extras (`join_type`, `reversible`, `where_sql`, aliases, declared
`relationship_type` when not `many_to_one`), non-reconstructible measures
(filtered, `percentile`/`list`/`*_distinct_on`, raw-SQL), and files with no
OSI form (query views, unrecognized files).
...
- a join `on_sql` is not an equi-join of `${view.field}` references (OSI
  relationships are equi-joins, so a non-equi or cross join has no OSI form);
...
- two names sanitize to the same Omni identifier (never silently merged);
- a measure has an unknown `aggregate_type`; an import directory has no view
  files; the input YAML is malformed.
...
- One generated topic per OSI model; multi-path (aliased) join fan-out and
  Omni query views, composite topics, access grants/filters, and templated
  filters are stash-and-restore only (no OSI semantics).
