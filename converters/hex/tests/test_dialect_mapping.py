...
def test_map_hex_dialect_is_case_insensitive_and_falls_back_to_ansi() -> None:
    assert map_hex_dialect_to_ossie("SNOWFLAKE") == OSIDialect.SNOWFLAKE
    assert map_hex_dialect_to_ossie("unknown") == DIALECT_ANSI
    assert map_hex_dialect_to_ossie(HexDialect.DUCKDB) == DIALECT_ANSI
