...
See ``ARCHITECTURE.md`` in this package for the closure contract and the
state invariants, and ``core-spec/foundational_semantics.md`` §6 (the
authoritative spec) for operator semantics, grain contracts, and laws.
...
Mutation-score target: ≥ 90% (see ``ARCHITECTURE.md`` in this package). A
surviving mutation in this module is a P0.
"""

from ossie.algebra.composition import add_columns, broadcast
...
from ossie.algebra.joins import filtering_join, merge
from ossie.algebra.operations import (
    FilterMode,
    JoinType,
    aggregate,
    enrich,
