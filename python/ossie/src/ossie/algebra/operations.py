...
``filtering_join`` — live in :mod:`ossie.algebra.joins`; the
two scalar-composition operators — ``add_columns`` and ``broadcast``
— live in :mod:`ossie.algebra.composition`. The split keeps
each file inside the 600-line per-file budget (``ARCHITECTURE.md``);
all nine operators share the same contract and are re-exported
through :mod:`ossie.algebra`.

Every compiler transformation is expressed as a composition of these
nine operators. Adding a tenth is a SPEC change (see
``ARCHITECTURE.md``).

Mutation-score target: **≥ 90%** for this module (``ARCHITECTURE.md``). A
...
from ossie.algebra.state import (
    AggregateFunction,
    AggregateInfo,
    CalculationState,
    Column,
...
from ossie.common.types import DimensionSet
...
class FilterMode(StrEnum):
    ...
    operator is defined in :mod:`ossie.algebra.joins` to keep
...
def source(
    ...
    * every set in ``unique_keys`` is non-empty and references
      ...
      unique-but-not-the-PK. ``core-spec/foundational_semantics.md §4.2`` and
...
def filter_(
    ...
    *,
    ...
    :class:`CalculationState` (``ARCHITECTURE.md``); a predicate
...
def enrich(
    ...
    * ``child.is_unique_on(child_keys)`` — i.e. ``child`` is *unique
      ...
      grain or any declared :attr:`unique_key`. ``core-spec/foundational_semantics.md
      ...
      :attr:`ErrorCode.E4001_EXPLOSION_UNSAFE` — the algebra's internal
      fan-out safety signal (``E4xxx`` family). The planner maps this
      signal to the user-facing code the query shape calls for:
      :attr:`ErrorCode.E_UNSAFE_REAGGREGATION` for a 1:N fan-trap in an
      aggregation query (Appendix A / D-022), or
      :attr:`ErrorCode.E_FAN_OUT_IN_SCALAR_QUERY` for a scalar query
      (D-023). Declared N:N edges are normally rejected earlier in the
      planner's path resolution with
      :attr:`ErrorCode.E3012_MN_NO_SAFE_REWRITE` or
      :attr:`ErrorCode.E3013_NO_STITCHING_DIMENSION`.
      :attr:`ErrorCode.E3011_MN_AGGREGATION_REJECTED` is reserved for
      the engine-wide "no M:N support" opt-out (§6.8) and is never
      raised by the algebra.
    ...
    * no child column is an aggregate (aggregates are built by
    ...
    # (the bridge-resolution mid-pipeline plan, ``§6.8.1``, depends on
    ...
    incoming = tuple(
        (
            replace(
                ...
                # Mark it discharged so a later holistic aggregate over it is
                # rejected (Semantic 4). Plain enriched dims (the ``else``
                # branch) are NOT marked — a single-step holistic over them is
                # well-defined per D-020.
                is_discharged_aggregate=True,
            )
...
def aggregate(
    ...
    * every ``new_grain`` column is **either** a member of
      ...
      N:1 join (see the evaluation algorithm in
      ``core-spec/foundational_semantics.md §6.2``).
    ...
    * if an aggregation reads a **discharged** aggregate column (a value
      already reduced at a finer grain and brought in by :func:`enrich` —
      ``is_discharged_aggregate``), it is not a ``HOLISTIC`` aggregate
      (``COUNT DISTINCT`` etc.); re-aggregating a discharged value
      holistically is unsafe (``E4001 EXPLOSION_UNSAFE``). A holistic over a
      *plain* enriched dimension is allowed — it is a single-step aggregate
      over the non-fanned joined rows, well-defined per D-020.
    """
    ...
    for agg in aggregations:
        ...
        if agg.aggregate.function.decomposability is Decomposability.HOLISTIC:
            for dep in agg.dependencies:
                ...
                if src.is_discharged_aggregate:
                    raise AlgebraError(
                        ErrorCode.E4001_EXPLOSION_UNSAFE,
                        f"holistic aggregation {agg.name!r} reads discharged "
                        f"(already-aggregated) column {dep!r}; a holistic "
                        "aggregate cannot be re-applied to a pre-aggregated "
                        "value — aggregate at the finer grain first",
