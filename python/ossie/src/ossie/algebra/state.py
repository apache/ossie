...
``ossie.parsing`` or ``ossie.codegen``; those layers see algebra values but
never construct them directly. Construction happens only through
:func:`ossie.algebra.operations.source` (and its downstream
operator chain).
...
from ossie.common.types import DimensionSet, ExpressionId
...
class ColumnKind(StrEnum):
    ...
    .. note::
       ...
       parser-level :class:`~ossie.parsing.models.FieldKind` does (see
       ...
       grains — are deferred features (see `core-spec/foundational_semantics.md §10`) and have
...
class Column:
    ...
    # True when this column carries a value that has ALREADY been aggregated at
    # a finer grain and then discharged to a scalar by :func:`enrich` (the
    # bridge mid-pipeline plan). Such a value must not be re-aggregated by a
    # holistic function — that would be an unsafe re-aggregation (Semantic 4).
    # A plain dimension/fact brought in by an N:1 enrich is NOT discharged: a
    # single-step holistic over it is one SQL aggregate over the (non-fanned)
    # joined rows, well-defined per D-020, so it stays allowed.
    is_discharged_aggregate: bool = False
...
class CalculationState:
    ...
    Constructed only by :mod:`ossie.algebra.operations` — never
    ...
    def is_unique_on(self, keys: DimensionSet) -> bool:
        ...
        Used by :func:`ossie.algebra.enrich` to discharge the
