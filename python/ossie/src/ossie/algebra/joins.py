...
per-file line budget (``ARCHITECTURE.md``, 600 lines). Everything here is
...
from ossie.algebra.operations import FilterMode
from ossie.algebra.state import CalculationState
...
def merge(
    ...
    * if ``on`` is given, ``on == left.grain`` (the algebra spec
      ``ARCHITECTURE.md`` defines merging on the shared
