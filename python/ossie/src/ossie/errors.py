...
See ``core-spec/foundational_semantics.md`` (Appendix A: Error Code Index)
for the full catalog. Every code listed there must have an enum value here
before it can be raised in production code.
...
class ErrorCode(StrEnum):
    """Stable error codes. See ``core-spec/foundational_semantics.md (Appendix A)``.

    A code marked ``RESERVED`` is documented in ``core-spec/foundational_semantics.md (Appendix A)``
    ...
    # E_* — Foundation v0.1 named codes (Appendix A of
    # ``core-spec/foundational_semantics.md``). The Foundation rollout (S-1..S-17)
    ...
    # §10's grain-aware-functions proposal (core-spec/foundational_semantics.md
    ...
    # :func:`ossie.planning.steps.source_step` and
    # :func:`ossie.planning.columns.compute_field_dependencies`.
    ...
    # :mod:`ossie.planning.planner_scalar`.
    ...
    # aggregation branch — see ``ARCHITECTURE.md`` I-43). The code stays in
    # the enum because Appendix A requires it and so the future
    ...
    # validator live in :mod:`ossie.parsing.function_whitelist`.
    ...
    # engine identified. (Appendix A / D-025.)
    E_AMBIGUOUS_MEASURE_GRAIN = "E_AMBIGUOUS_MEASURE_GRAIN"
    # RESERVED — Appendix A / §4.2. Engines that opt to require
    ...
    # RESERVED — Appendix A. Declared by a model that uses the
    ...
    # ``core-spec/foundational_semantics.md`` §3
    ...
    # above — see ``core-spec/foundational_semantics.md`` §3.
    ...
    # ``core-spec/foundational_semantics.md §6.4``.
    ...
    # (engine-wide, never per-query — see ``core-spec/foundational_semantics.md``
    # §6.8 and Appendix A). This reference implementation is
    # M:N-supporting, so it never raises E3011; per-query M:N failures
    # use ``E3012`` / ``E3013``, and the algebra's internal fan-out
    # signal is ``E4001_EXPLOSION_UNSAFE`` (not E3011).
    ...
    # stitch on. See ``core-spec/foundational_semantics.md`` §6.8.
    E3012_MN_NO_SAFE_REWRITE = "E3012"
    E3013_NO_STITCHING_DIMENSION = "E3013"

    # E4xxx — Algebra safety errors (raised only by ossie.algebra; the
    # planner translates them to the user-facing E_* codes). E4001 is the
    # fan-out / explosion signal: a source column that is already an
    # aggregate, an enrich whose child is not unique on the join keys
    # (fan-trap), or a holistic aggregate over a discharged join-RHS
    # value. See ``ossie.algebra`` and Semantic 2 / Semantic 4 (§6.1).
...
class OSIParseError(OSIError):
    """Raised from ``ossie.parsing``. Codes in ``E1xxx`` / ``E2xxx``."""


class OSIPlanningError(OSIError):
    """Raised from ``ossie.planning`` (outside the algebra). Codes in ``E3xxx``."""


class AlgebraError(OSIError):
    """Raised from ``ossie.algebra``.
...
class OSICodegenError(OSIError):
    """Raised from ``ossie.codegen``. Codes in ``E5xxx``."""


class OSIWarning(OSIError):  # noqa: N818 — a warning, deliberately not "*Error"
