...
def parse_sql_expr(source: str) -> exp.Expression:
    ...
    # sqlglot's parse_one is typed to return its internal ``Expr`` alias; it is
    # an ``Expression`` in practice (and we've ruled out None above).
